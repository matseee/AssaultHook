...
bool InjectLibrary(uint processIdentifier, const char *pathToLibrary) {
  memory::Module mod = memory::GetModule("linux_64_client", processIdentifier);
...
int main() {
  ...
  std::cout << "Looking for AssaultCube ..." << std::endl;
  uint ac_client = 0;
  while (!ac_client) {
  ...
  std::cout << "Injecting library to process " << ac_client << " ..."
            << std::endl;
